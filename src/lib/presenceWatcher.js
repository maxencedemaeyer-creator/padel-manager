...
async function autoAbsentUnplacedPresences(matches) {
  try {
    ...
    for (const session of groupMatchesBySession(matches)) {
      ...
      session.forEach((m) =>
        // Array.isArray(...) : un très ancien match dont le champ
        // "participants" ne serait pas un tableau ne doit pas faire planter
        // toute l'app — voir la même précaution dans src/lib/stats.js.
        (Array.isArray(m.participants) ? m.participants : []).forEach((p) =>
