...
// manuel de la page (bouton dédié du navigateur), mais une nouvelle connexion
// (nouvel onglet/navigateur) repart bien sur "Matchs" par défaut, comme voulu
// à l'origine (voir plus haut, commentaire sur l'import direct de Matchs).
const VIEW_STORAGE_KEY = "pm-active-view";
const VALID_VIEWS = ["matches", "players", "stats", "accounting", "game-center", "admin"];
...
function MainApp() {
  ...
  const [view, setView] = useState(() => {
    try {
      ...
      // Un onglet restauré auquel ce joueur n'a plus droit (changement de
      // rôle, Game Center désactivé entre-temps...) ne doit jamais
      // s'afficher par erreur — on retombe sur "Matchs".
      if (saved === "admin" && !appData.isAdmin) return "matches";
      if (saved === "accounting" && !appData.connectedPlayer.isCreditor) return "matches";
      if (saved === "game-center" && !appData.isAdmin && !settingsHook.settings.gameCenterEnabled) {
        return "matches";
      }
      return saved;
    } catch {
  ...
  // Change d'onglet ET mémorise le choix (sessionStorage) pour qu'un
  // rafraîchissement de la page y revienne — voir VIEW_STORAGE_KEY plus haut.
  const changeView = (id) => {
    setView(id);
    try {
      ...
      // changement d'onglet fonctionne quand même, seule la mémorisation
  ...
  const contextValue = useMemo(
    () => ({
      ...
      // (19/09/2026 : pointe désormais sur changeView, pour que ce genre de
      // navigation depuis un lien interne soit aussi mémorisée au rechargement.)
      setView: changeView,
    }),
    [
      appData,
